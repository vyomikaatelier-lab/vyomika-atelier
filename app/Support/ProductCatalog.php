...
class ProductCatalog
{
    ...
    /** Auto-generated catalog filler SKUs from generate_service_gallery_products(). */
    public static function isAutoGeneratedCatalogSku(?string $sku): bool
    {
        if ($sku === null || $sku === '') {
            return false;
        }

        return (bool) preg_match('/^SSM-P\d/', $sku)
            || (bool) preg_match('/^SSM-(SD|ED|RK|BF)\d{3}$/', $sku);
    }

    /** @param array<string, mixed> $item */
    public static function isCatalogFillerItem(array $item): bool
    {
        $name = $item['name'] ?? '';
        $image = $item['image'] ?? '';
        $sku = $item['sku'] ?? '';

        if (! filled($name) || ! filled($image)) {
            return true;
        }

        if (str_contains((string) $image, 'unsplash.com')) {
            return true;
        }

        return self::isAutoGeneratedCatalogSku(is_string($sku) ? $sku : null);
    }
