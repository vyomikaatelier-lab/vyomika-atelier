...
class Product extends Model
{
    ...
    public function isCatalogFiller(): bool
    {
        ...
        return ProductCatalog::isAutoGeneratedCatalogSku($this->sku);
    }
    /** Skip gallery cards that would render the VA placeholder (missing/broken local media). */
    public function hasDisplayableGalleryImage(): bool
    {
        if ($this->isCatalogFiller()) {
            return false;
        }
        ...
        if (str_starts_with($path, 'http://') || str_starts_with($path, 'https://') || str_starts_with($path, '//')) {
            return ! str_contains($path, 'unsplash.com');
        }

        if (str_starts_with($path, '/storage/') || str_starts_with($path, 'storage/')) {
            $relative = ltrim(substr($path, str_starts_with($path, '/storage/') ? 9 : 8), '/');
