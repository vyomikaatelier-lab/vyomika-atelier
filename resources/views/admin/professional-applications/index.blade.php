...
<table class="w-full bg-white rounded-lg shadow text-sm">
    <thead class="border-b"><tr class="text-left"><th class="p-3">Name</th><th class="p-3">Email</th><th class="p-3">Protection</th><th class="p-3">Risk</th><th class="p-3">Status</th><th class="p-3">Submitted</th><th class="p-3"></th></tr></thead>
    <tbody>@foreach($applications as $app)<tr class="border-b">
        <td class="p-3">{{ $app->name }}</td><td class="p-3">{{ $app->email }}</td>
        <td class="p-3">{{ $app->protectionStatusLabel() }}</td><td class="p-3">{{ $app->risk_score }}/100</td>
        <td class="p-3">{{ $app->statusLabel() }}</td><td class="p-3">{{ $app->created_at->format('d M Y') }}</td>
