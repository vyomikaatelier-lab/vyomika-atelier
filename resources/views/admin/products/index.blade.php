...
<table class="w-full bg-white rounded-lg shadow text-sm">
    ...
    <tbody id="product-sortable-list">
        ...
        <tr class="border-b {{ ! $product->isClassified() ? 'bg-amber-50' : '' }}" data-product-id="{{ $product->id }}">
            ...
            <td class="p-3">
                @if($product->imageUrl())
                <img src="{{ $product->imageUrl() }}" alt="" class="w-12 h-12 object-cover rounded border bg-gray-50">
                @else
                <span class="inline-flex w-12 h-12 items-center justify-center rounded border bg-gray-100 text-xs text-gray-400">—</span>
                @endif
            </td>
        ...
        @empty
        <tr>
            <td colspan="11" class="p-6 text-center text-gray-500">No products match this filter.</td>
        </tr>
        @endforelse
    </tbody>
</table>
