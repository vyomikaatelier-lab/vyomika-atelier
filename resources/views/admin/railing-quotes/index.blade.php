...
<table class="w-full bg-white rounded-lg shadow text-sm">
    <thead class="border-b"><tr class="text-left"><th class="p-3">Name</th><th class="p-3">Dimensions</th><th class="p-3">Status</th><th class="p-3">Submitted</th><th class="p-3"></th></tr></thead>
    <tbody>@foreach($quotes as $quote)<tr class="border-b">
        <td class="p-3">{{ $quote->name }}</td><td class="p-3">{{ $quote->dimensions }}</td><td class="p-3">{{ $quote->statusLabel() }}</td><td class="p-3">{{ $quote->created_at->format('d M Y') }}</td>
