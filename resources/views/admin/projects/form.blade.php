...
<form method="POST" action="{{ isset($project) ? route('admin.projects.update', $project) : route('admin.projects.store') }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow space-y-4 max-w-3xl">
    ...
    <div><label class="block text-sm mb-1">Gallery URLs (one per line)</label><textarea name="gallery_urls" rows="4" class="w-full border px-3 py-2 rounded">{{ old('gallery_urls', isset($project) && is_array($project->gallery) ? implode("\n", $project->gallery) : '') }}</textarea></div>
    <div class="grid md:grid-cols-2 gap-4">
